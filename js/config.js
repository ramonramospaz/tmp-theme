jQuery(document).ready(function($){
	...
	$(function() {
        ...
        $('#addScnt').on('click', function() {
			var row = $("#row").val();
			$('<div class="group"><h2><label for="p_scnts"><input type="text" class="p_scnt" size="20" row="'+row+'" name="eccs[]" value="" placeholder="Elemento complementario clave de su servicio" /></label> </h2><h3><select row="'+row+'" name="ieccs[]"><option value="0">Seleccione una opción</option><option value="1">Poco importante</option><option value="2">Ligeramente importante</option><option value="3">Medianamente importante</option><option value="4">Muy importante</option><option value="5">Sumamente importante</option><option value="6">No aplica</option><option value="7">No sabe</option></select><button type="button" class="rmvScnt">Eliminar</button></h3></div>').appendTo(scntDiv);
			i++;
			row++;
			$("#row").val(row);
			return false;
