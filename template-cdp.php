<?php /* Template Name: Competencias distintivas producto*/
	...
	if(mysqli_num_rows($query_buscar) > 0){
		...
		for($i=0;$i<$n;$i++){
			if(($array[$array_keys[$i]] == '4') || ($array[$array_keys[$i]] == '5')){
				$query_preguntas = mysqli_query($conexion, "SELECT id, pregunta, rel FROM wp_tmp_preguntas WHERE name = '".$array_keys[$i]."'");
...
<?php get_header(); ?>
	...
	<div class="wrapper">
		...
		<h1><?php the_title(); ?></h1>
		<p>Evalue cuan COMPETENTE (habilidad, dominio y destreza) es SU MARCA Y/O PRODUCTO en cuanto a cada uno de los FACTORES mencionados.</p>	
	
	<?php if (have_posts()): while (have_posts()) : the_post(); ?>
	
		<article id="post-<?php the_ID(); ?>" <?php post_class(); ?>>

			<form method="post" id="cdproducto">
