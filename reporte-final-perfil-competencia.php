<?php /* Template Name: R Final perfil competencia */
	...
	$query_cc = mysqli_query($conexion, "SELECT pregunta_id, respuesta FROM wp_tmp_caractercompetencia WHERE solicitud_id = '".$_SESSION["solicitud_id"]."'") or die(mysqli_error($conexion));
	...
	if(mysqli_num_rows($query_buscar) > 0){
		...
		for($i=0;$i<$n;$i++){ 
			...
			/* 1 */
			$query_preguntas_a = mysqli_query($conexion, "SELECT pregunta FROM wp_tmp_preguntas WHERE name = '".$array_keys[$i]."'") or die(mysqli_error($conexion));
			...
			if(($array[$array_keys[$i]] != '4') && ($array[$array_keys[$i]] != '5')){
				...
				$query_preguntas = mysqli_query($conexion, "SELECT pregunta, respuesta FROM wp_tmp_preguntas INNER JOIN wp_tmp_cdcompetencia ON wp_tmp_preguntas.id = wp_tmp_cdcompetencia.pregunta_id WHERE solicitud_id = '".$_SESSION["solicitud_id"]."' AND name = '".$array_keys[$i]."'") or die(mysqli_error($conexion));
	...
	for($i=0;$i<$s;$i++){
		...
		$tabla_desempeño = '<table class="desempeno">
				<tr>
					<th colspan="2">Desempeño de acciones de la competencia</th>
				</tr>
				<tr>
...
<?php get_header(); ?>
	...
	<?php if (have_posts()): while (have_posts()) : the_post(); ?>
		...
		<article id="post-<?php the_ID(); ?>" <?php post_class(); ?>>
			...
			<?php echo $tabla_cc; echo $tabla_desempeño; ?>
